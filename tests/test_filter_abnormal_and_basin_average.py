...
def test_calc_filter_station_list():
    ...
    era5_sl_diff_df = pd.read_csv(
        os.path.join(definitions.ROOT_DIR, "example/era5_sl_diff.csv")
    ).rename(columns={"Unnamed: 0": "STCD"})
    era5_biliu_diff_df = pd.read_csv(
        os.path.join(definitions.ROOT_DIR, "example/era5_biliu_diff.csv")
    ).rename(columns={"Unnamed: 0": "STCD"})
    biliu_hourly_splited_path = os.path.join(
        definitions.ROOT_DIR, "example/biliu_history_data/history_data_splited_hourly"
    )
    biliu_hourly_filtered_path = os.path.join(
        definitions.ROOT_DIR, "example/filtered_data_by_time"
    )
...
def test_filter_abnormal_sl_and_biliu():
    biliu_his_stas_path = os.path.join(
        definitions.ROOT_DIR, "example/biliu_history_data/history_data_splited_hourly"
    )
    sl_biliu_stas_path = os.path.join(definitions.ROOT_DIR, "example/rain_datas")
    time_df_dict_biliu_his = get_filter_data_by_time(
        biliu_his_stas_path, filter_station_list
    )
    ...
    for key in space_df_dict.keys():
        space_df_dict[key].to_csv(
            os.path.join(
                ...
                "example/filtered_rain_between_sl_biliu",
                key + "_filtered.csv",
            )
        )
...
def get_filter_data_by_time(data_path, filter_list=None):
    if filter_list is None:
        filter_list = []
    time_df_dict = {}
    test_filtered_by_time_path = os.path.join(
        definitions.ROOT_DIR, "example/filtered_data_by_time"
    )
    for dir_name, sub_dir, files in os.walk(data_path):
        for file in files:
            ...
            if (
                (int(stcd) not in filter_list)
                & (~os.path.exists(cached_csv_path))
                & (feature != "水位")
            ):
                drop_list = []
                ...
                if "DRP" in table.columns:
                    table["TM"] = pd.to_datetime(
                        table["TM"], format="%Y-%m-%d %H:%M:%S"
                    )
                    table = table.drop(columns=["Unnamed: 0"]).drop(
                        index=table.index[table["DRP"].isna()]
                    )
...
def get_filter_data_by_space(time_df_dict, filter_list):
    neighbor_stas_dict = find_neighbor_dict(
        sl_stas_table, biliu_stas_table, filter_list
    )[0]
        ...
        for time in table.index:
            ...
            if len(rain_time_dict) == 0:
                ...
                for sta in rain_time_dict.keys():
                    point = gdf_stid_total.geometry[
                        gdf_stid_total["STCD"] == str(sta)
                    ].values[0]
                    point_self = gdf_stid_total.geometry[
                        gdf_stid_total["STCD"] == str(key)
                    ].values[0]
                ...
                if "DRP" in table.columns:
                    MA_Tct = (table["DRP"][time] - average) / (
                        quantile_75 - quantile_25
                    )
                    if MA_Tct > 4:
                        time_drop_list.append(time)
                elif "paravalue" in table.columns:
                    MA_Tct = (table["paravalue"][time] - average) / (
                        quantile_75 - quantile_25
                    )
                    if MA_Tct > 4:
                        time_drop_list.append(time)
...
def find_neighbor_dict(sl_biliu_gdf, biliu_stbprp_df, filter_list):
    biliu_stbprp_df = (
        biliu_stbprp_df[biliu_stbprp_df["sttp"] == 2]
        .reset_index()
        .drop(columns=["index"])
    )
    point_list = []
    ...
    gdf_biliu = GeoDataFrame(
        {"STCD": biliu_stbprp_df["stid"], "STNM": biliu_stbprp_df["stname"]},
        geometry=point_list,
    )
    ...
    gdf_stid_total = (
        gdf_stid_total.set_index("STCD").drop(index=filter_list).reset_index()
    )
    gdf_stid_total["STCD"] = gdf_stid_total["STCD"].astype("str")
    neighbor_dict = {}
    for i in range(0, len(gdf_stid_total.geometry)):
        stcd = gdf_stid_total["STCD"][i]
        gdf_stid_total["distance"] = gdf_stid_total.apply(
            lambda x: distance(gdf_stid_total.geometry[i], x.geometry), axis=1
        )
        nearest_stas = gdf_stid_total[
            (gdf_stid_total["distance"] > 0) & (gdf_stid_total["distance"] <= 0.2)
        ]
...
def get_voronoi_total():
    node_shp = os.path.join(
        definitions.ROOT_DIR, "example/biliuriver_shp/biliu_basin_rain_stas_total.shp"
    )
    dup_basin_shp = os.path.join(
        definitions.ROOT_DIR, "example/biliuriver_shp/碧流河流域_副本.shp"
    )
    origin_basin_shp = os.path.join(
        definitions.ROOT_DIR, "example/biliuriver_shp/碧流河流域.shp"
    )
    if not os.path.exists(node_shp):
        shutil.copyfile(origin_basin_shp, dup_basin_shp)
        gdf_stid_total = find_neighbor_dict(
            sl_stas_table, biliu_stas_table, filter_list=filter_station_list
        )[1]
...
def test_rain_average_filtered(
    start_date="2014-01-01 00:00:00", end_date="2022-09-01 00:00:00"
):
    ...
    voronoi_gdf["real_area"] = voronoi_gdf.apply(
        lambda x: x.geometry.area * 12100, axis=1
    )
    rain_path = os.path.join(
        definitions.ROOT_DIR, "example/filtered_rain_between_sl_biliu"
    )
    table_dict = {}
    ...
    for time in pd.date_range(start_date, end_date, freq="H"):
        ...
        for stcd in time_rain_records.keys():
            ...
            rain_aver += (
                time_rain_records[stcd]
                * voronoi_gdf["real_area"][voronoi_gdf["STCD"] == stcd].values[0]
                / gdf_biliu_shp["area"][0]
            )
        rain_aver_dict[time] = rain_aver
    rain_aver_df = pd.DataFrame(
        {"TM": rain_aver_dict.keys(), "rain": rain_aver_dict.values()}
    )
    rain_aver_df.to_csv(
        os.path.join(definitions.ROOT_DIR, "example/filtered_rain_average.csv")
    )
    return rain_aver_dict


def get_infer_inq():
    inq_csv_path = os.path.join(
        definitions.ROOT_DIR, "example/biliu_inq_interpolated.csv"
    )
    if os.path.exists(inq_csv_path):
        new_df = pd.read_csv(inq_csv_path, engine="c", parse_dates=["TM"]).set_index(
            "TM"
        )
    else:
        biliu_flow_df = pd.read_csv(
            os.path.join(definitions.ROOT_DIR, "example/biliuriver_rsvr.csv"),
            engine="c",
            parse_dates=["TM"],
        )
        ...
        for i in range(1, len(biliu_flow_df)):
            if (int(inq_array[i]) == -1) & (int(otq_array[i]) != -1):
                ...
                time_div = np.timedelta64(
                    tm_array[i] - tm_array[i - 1]
                ) / np.timedelta64(1, "h")
...
def biliu_rain_flow_division():
    ...
    # 从自动划分结果里手动选几个场次
    session_times = [
        ("2017/8/1 15:00:00", "2017/8/7 07:00:00"),
        ("2018/8/19 12:00:00", "2018/8/23 09:00:00"),
        ("2020/8/31 04:00:00", "2020/9/4 15:00:00"),
        ("2022/7/6 10:00:00", "2022/7/10 00:00:00"),
    ...
    for session in session_times:
        ...
        session_df = pd.DataFrame(
            {
                "TM": pd.date_range(start_time, end_time, freq="H"),
                "RAIN_SESSION": rain_session.to_numpy().flatten(),
...
def test_calibrate_flow():
    # pet_df含有潜在蒸散发
    pet_df = pd.read_csv(
        os.path.join(definitions.ROOT_DIR, "example/era5_data/pet_calc/PET_result.CSV"),
        engine="c",
        parse_dates=["time"],
    ).set_index("time")
    ...
    for session_df in session_df_list:
        ...
        calibrate_df = pd.DataFrame(
            {
                "PRCP": session_df["RAIN_SESSION"].to_numpy(),
                "PET": session_pet,
...
def test_compare_paras():
    ...
    for session_df in sessions_list:
        ...
        session_df = pd.concat(
            [
                session_df["RAIN_SESSION"],
                pet_session,
                session_df["FLOW_SESSION_MM_H"],
                session_df["FLOW_SESSION_M3_S"],
            ],
            axis=1,
        )
        session_np = session_df.to_numpy()
        session_np = np.expand_dims(session_np, axis=1)
        qsim, es = hydromodel.models.xaj.xaj(
            p_and_e=session_np[:, :, 0:2],
        ...
        plt.savefig(
            os.path.join(
                definitions.ROOT_DIR,
                "example/deap_dir/calibrated_xaj_cmp",
                time_dir + ".png",
            )
        )
        print(rmse)
